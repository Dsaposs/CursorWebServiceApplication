...
const selectedTemplate = computed(() => findNpcTemplate(props.definition, selectedTemplateKey.value));
...
function applyTemplate(templateKey: string) {
  if (!templateKey || !props.definition) {
    return;
  }

  const template = findNpcTemplate(props.definition, templateKey);
  if (!template) return;

  const applied = applyNpcTemplateToForm(template, props.definition, attributes.value, skills.value);
